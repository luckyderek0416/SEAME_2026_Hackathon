...
class LaneDetector:
    def __init__(self, roi_top_ratio=0.55, bright_thresh=160, min_pixels=40,
                 ...
                 crossline_sw_gate=0, sw_entry_frames=0,
        ...
        self._roundabout_exit_context = False
        ...
        self.crossline_sw_gate = crossline_sw_gate                  # gate to enter the roundabout-exit fallback state
        self.sw_entry_frames = max(0, sw_entry_frames)              # how long to hold the fallback state after the gate
        self._sw_entry_count = 0                                     # frames remaining in the fallback hold
    ...
    def process(self, bgr, draw_debug=True):
        ...
        if bands:
            ...
            # Conservative single-line fallback: only apply it when we have a single
            # detected line and the current steering tendency suggests a clear side.
            if (num_lanes == 1 and bands and self._roundabout_exit_context
                    and abs(curvature) >= 0.25):
                lane_center = self._single_line_center_with_context(bands, w)
        ...
        self._update_roundabout_exit_context(yellow_crossline)
    ...
    def _update_roundabout_exit_context(self, yellow_crossline):
        """Hold an exit-context state briefly after the yellow crossline gate.

        This makes the single-line fallback remain active for a short post-gate period,
        which is useful when the roundabout exit briefly narrows to a single visible lane.
        """
        if self.crossline_sw_gate > 0 and yellow_crossline:
            self._sw_entry_count = self.sw_entry_frames
            self._roundabout_exit_context = True
            return

        if self._sw_entry_count > 0:
            self._sw_entry_count -= 1
            if self._sw_entry_count <= 0:
                self._roundabout_exit_context = False
            return
    ...
    def _should_use_single_line_context(self, curvature, roundabout_exit_context):
        """Only enable the steering-aware single-line logic in a tight roundabout-exit
        context: the detector must be in the exit window and the curvature should be
        strong enough that the lane is likely to be a single exiting branch."""
        return bool(roundabout_exit_context and abs(curvature) >= 0.25)

    def _single_line_center_with_context(self, bands, w):
        """When only one lane line is visible, infer whether it is the left or right
        lane from the current steering tendency and offset the estimated lane centre
        accordingly. This is meant for the RA->DRIVE transition where a single dashed
        line may still be followed by the SW path."""
        if not bands:
            return None
        near_cx = bands[0][0]
        half_width = self._lane_width / 2.0 if self._lane_width > 0 else (w * self.single_line_offset)
        if half_width <= 0:
            half_width = w * self.single_line_offset
        if self._last_steer > 0.0:
            # steering right (positive) means the car is drifting to the left; treat the
            # visible line as the left-side lane and place the lane center to its right.
            return near_cx + half_width
        if self._last_steer < 0.0:
            # steering left (negative) means the car is drifting to the right; treat the
            # visible line as the right-side lane and place the lane center to its left.
            return near_cx - half_width
        return near_cx
