...
- 코드 정리(dead-path)는 레이스 후. 확정 후보:
  - `ra_entry/exit_oneline_frames` (run31 off 결정, SW 코리도가 대체 — 단 폴백 보험 가치는 레이스까지 유지)
  - 표결의 yaw 표(`yaw_done`/`yaw_lap_threshold`) — 7.0 봉인으로 사실상 무기능, cross+time 2표가 실질 전부
  - `_reacq_right_active` 계열, `yellow_heading_gain`(0=off) 등 off 확정 파라미터 일괄
