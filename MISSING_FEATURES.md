...
### A-1. ✅ BEV 재캘리브레이션 — **완료 (2026-07-07 실측)**

직선 구간에 차를 가운데 두고 실프레임 캡처 → 좌/우 흰선 직선 피팅으로 src 좌/우 변을
실제 차선 위에 정렬. 검증: 워프 후 두 차선 slope ±0.05(거의 수직), x=0.20w/0.80w 평행.
- 확정값: `roi_top_ratio=0.35` (하단 65% 사용) /
  `birdeye_src_ratio=[0.226,0.342, 0.745,0.342, 0.998,0.965, -0.006,0.965]`
  (TL/TR x 는 실측 후 수동 미세조정). ⚠️ **카메라 마운트 다시 만지면 재실행.**
...
**인코스 플로우(2026-07-07 재설계):** 초록불→흰선 추종 DRIVE → 노란색이 보이면
(≥`follow_yellow_ratio`) **노란선만 추종**(FOLLOW-Y) → 노란 가로선(정지선) **처음** 보이면
ROUNDABOUT on → 같은 가로선을 **한 번 더**(`roundabout_exit_gates`=1) 만나면 출구 브랜치
락온으로 탈출(정방향=우회전) → 노란 추종 유지 → **흰색>노란색** 검출되면 흰선 추종 복귀.
ROUNDABOUT 동안은 강제 노란 추종(바깥 흰 루프 오검 방지). 기존 안전장치 유지:
`min_loop_time_s`(3s) HARD floor, 쿨다운 디바운스, 2-of-4 표결 failsafe, `max_loop_time_s`.
...
**빨간불 오인식 가드 (07-07 추가)** — 주행 중 red_light 검출(멀리 보이는 진짜 도착
신호등 or 오분류)로 코스 중간에 FINISH/DONE 되던 사고 대응:
- `finish_min_drive_s`(15s): 출발 후 이 시간 전엔 빨간불 완전 무시. **코스 소요시간
  실측해서 여유 있게 재설정** (너무 길면 진짜 도착 때 못 멈춤). 라이브 가능.
...
| `steer_left_gain` | 1.2 | 좌조향 비대칭 보정 (좌로 덜 꺾이는 하드웨어 특성 증폭) |
| `finish_min_drive_s` | 15.0 | 출발 후 빨간불 무시 시간 (B-5 가드) |
...
`branch_bias`, `yaw_lap_threshold`, `nominal_loop_time_s`, `race_dir`, `course`
